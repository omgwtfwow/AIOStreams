...
Current installs use session-based login and a config access key. Configure
logins with `AIOSTREAMS_AUTH` and set `CONFIG_ACCESS_KEY` only when you need to
manage the key explicitly.
